...
# from dataset.dataset_ri import SSGCMFeatRIDataset
from dataset.database import SSGCMFeatDataset
from model.frontend.pointnet import PointNetEncoder, feature_transform_reguliarzer
from model.loss import IntraModalBarlowTwinLoss, SupervisedCrossModalInfoNCE, SupervisedCrossModalNXTent, SupervisedTextModalInfoNCEOriginal
...
def train(rank): # , world_size
    ...
    # )
    # np.random.seed(config_system["SEED"])
    ...
    # SupervisedCrossModalNXTent
    # SupervisedTextModalInfoNCEOriginal
    cm_visual_criterion = SupervisedCrossModalInfoNCE(device, temperature=0.07) # 0.07 # magnify temperature to 0.2
    ...
    for epoch in range(epoches):
        ...
        wandb_log = {}
        topk_obj_list = np.array([])
        for i, (data, data_2, rgb_imgs, zero_mask, text_feat, gt_label) in enumerate(train_loader):
            data, rgb_imgs, zero_mask, text_feat, gt_label = \
                data.to(rank), \
                rgb_imgs.to(rank), \
                zero_mask.to(rank), \
                text_feat.to(rank), \
            ...
            t_log = [
                ...
                ("train/imid_loss", l_reg.detach().cpu().item()),
        ...
        wandb_log['Train/Regularization'] = train_imid_losses.avg
