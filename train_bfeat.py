...
from model.loss import NegativeCosineSimilarity, NTXentLoss
...
def train(rank, world_size):
    ...
    for epoch in range(epoches):
        ...
        for i, (data_t1, data_t2, rgb_feat, text_feat, _) in enumerate(train_loader):
            data_t1, data_t2, rgb_feat = data_t1.to(device), data_t2.to(device), rgb_feat.to(device)
            ...
            loss_cmid = criterion(point_feats, rgb_feat)
        ...
        if rank == 0:
            ...
            for i, (data_v1, _, _, _, label) in enumerate(v_loader):
                ...
                data = data_v1.permute(0, 2, 1).to(device)
                with torch.no_grad():
